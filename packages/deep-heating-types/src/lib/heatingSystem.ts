...
/**
 * Effect Service for heating system device communication.
 *
 * This service expresses what the core heating logic needs from external
 * device communication, speaking purely in domain terms rather than external
 * system concepts (like Home Assistant entities).
 *
 * Implementations:
 * - HomeAssistantHeatingSystemLive: Wraps existing providers, translates HA entities
 * - InMemoryHeatingSystem: For testing, emits controllable fake domain data
 */
export class HeatingSystem extends Context.Tag('HeatingSystem')<
  HeatingSystem,
  {
    /**
