...
import deep_heating/state.{
  DeepHeatingState, RadiatorState, RoomState, TemperatureReading,
}
...
pub fn temperature_reading_construction_test() {
  let reading =
    TemperatureReading(temperature: temperature(21.5), time: 1_704_067_200)
  reading.time |> should.equal(1_704_067_200)
}

pub fn temperature_reading_temperature_field_test() {
  let reading =
    TemperatureReading(temperature: temperature(19.0), time: 1_704_067_200)
...
pub fn radiator_state_with_temperature_test() {
  let reading =
    TemperatureReading(temperature: temperature(20.0), time: 1_704_067_200)
  let radiator =
    RadiatorState(
      name: "Bedroom TRV",
...
pub fn deep_heating_state_empty_test() {
  let state = DeepHeatingState(rooms: [], is_heating: None)
...
pub fn deep_heating_state_with_rooms_test() {
  ...
  let state = DeepHeatingState(rooms: [room1, room2], is_heating: Some(True))
...
pub fn deep_heating_state_nested_structure_test() {
  let reading = TemperatureReading(temperature: temperature(20.5), time: 12_345)
  let trv =
    RadiatorState(
      name: "Lounge TRV",
      temperature: Some(reading),
  ...
  let state = DeepHeatingState(rooms: [room], is_heating: Some(True))
